...
function setOverallLeds(){
	...
	*/
	...
	/* This code is disabled because when mbf page is viewed as a remote node, it still affects the badge on the home icon of the host (OMU)
	// Show the number of red LEDs on badge
...
function processNmp(out,err){
	...
	break;
	default:
		console.debug("Could not parse the output of the NMP attribute: '"+out+"'");
	}
}
...
$(document).ready(function(e) {
	...
	axshCall(_N("get ihu"),function(out,err){
		...
		*/
		devicePresent( out, 0, ".psu1-component", "Master Power Supply" );
		devicePresent( out, 1, ".psu2-component", "Slave Power Supply"  );
		devicePresent( out, 2, ".ref-gen-component", "Reference Generator" );
		devicePresent( out, 3, ".band1", "Band 1" );
		devicePresent( out, 4, ".band2", "Band 2" );
		devicePresent( out, 5, ".band3", "Band 3" );
		devicePresent( out, 6, ".band4", "Band 4" );
		devicePresent( out, 7, ".fo1", "Fiber Optic Module 1" );
		devicePresent( out, 8, ".fo2", "Fiber Optic Module 2" );
		devicePresent( out, 9, ".fo3", "Fiber Optic Module 3" );
		devicePresent( out, 10,".fo4", "Fiber Optic Module 4" );
		devicePresent( out, 11, ".commux-component", "Communication Multiplexer (Commux)" );
		
		//ok now schedule repeated commands:
		axshCallInterval(30000,_N("get tag"),processTag);
