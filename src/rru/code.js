...
//                    $('.linkstate_cell').empty();
                    var rrustatus = $.parseJSON(o.ajaxdata);
                    //update rf table details
                    $.each(rrustatus.Bands,function(key, band){
            ...
            // flag2 RRU for hidden table_row 
                            ...
                            if (!_.isEmpty(operatorCellresList)) {
                                ...
                                $.each(operatorList, function (i, op) {
                                    ...
                                    if (!_.isEmpty(operatorCellresList[i])) {
                                        ...
                                        $.each(operatorCellresList[i], function (bandNo, band) {
                                            configBand('#' + operator.SysName + '_rru-band-freq-ul-indicator-' + bandNo, band[0].BandStartUL, band[0].BandStopUL, -110, 0); //rf level input for rru is from -110 to 0
                                            configBand('#' + operator.SysName + '_rru-band-freq-dl-indicator-' + bandNo, band[0].BandStartDL, band[0].BandStopDL, 0, 40);//rf level output for rru is from 0 to 40
