require([ '/general/general.js', '/js/console.js', '/js/api.js', '/js/scheduler.js', '/js/convert.js', '/js/util.js', '/js/lib/jquery.js', '/js/lib/underscore.js', '/js/lib/jquery-ui.js', '/js/lib/handlebars.js', '/js/handlebars-helpers.js', '/js/led.js','/js/lib/tipsy.js' ],
	...
	$(document).ready(function(){
            ...
            scheduler.add( sec(3), {
                cmd: 'rru40status --json',
                callOnDiff:true,
                onSuccess:function(o){
                    //refresh everything
//                    $('.deactivate_cell').empty();
                    var rru40status = $.parseJSON(o.ajaxdata);
                    //update rf table details
                    $.each(rru40status.Bands,function(key, band){
                        ...
                        setLedColor('#radio-'+band.No+'-monpafwd-led',band.MonPaFwd);
                        setLedColor('#radio-'+band.No+'-monparev-led',band.MonPaRev);
                        setLedColor('#radio-'+band.No+'-ampcurrent-led',band.AmpCurrent);
                        $('#radio-'+band.No+'-monpafwd-val').text(band.MonPaFwdLevel);
                        $('#radio-'+band.No+'-monparev-val').text(band.MonPaRevLevel);
                        $('#radio-'+band.No+'-ampcurrent-val').text(band.AmpCurrentLevel);
                            ...
                            if (!_.isEmpty(operatorCellresList)) {
                                    ...
                                    if (!_.isEmpty(operatorCellresList[i])) {
                                        ...
                                        $.each(operatorCellresList[i], function (bandNo, band) {
                                            tablehtml += ' <tr><th rowspan="2" id="' + operator.SysName + '_celres_band_' + bandNo + '" class="band-col"><div class="icon antenna2"></div> ' + bandNo + '</th>' +
                                                ...
                                                '<td id="' + operator.SysName + '_celres_band_ul_spectrum_' + bandNo + '"><div id ="' + operator.SysName + '_rru40-band-freq-ul-indicator-' + bandNo + '" class="freq-indicator"></div>' +
                                                '<div id ="' + operator.SysName + '_rru40-band-ul-table-' + bandNo + '"></div></td>' +
                                                '</tr>';
                                            tablehtml += '<tr><th class="uldl-col"><div class="icon downlink"></div> Downlink</th>' +
                                                '<td id="' + operator.SysName + '_celres_band_dl_spectrum_' + bandNo + '"><div id ="' + operator.SysName + '_rru40-band-freq-dl-indicator-' + bandNo + '" class="freq-indicator"></div>' +
                                                '<div id ="' + operator.SysName + '_rru40-band-dl-table-' + bandNo + '"></div></td>' +
                                                '</tr>';
                                ...
                                $.each(operatorList, function (i, op) {
                                    ...
                                    if (!_.isEmpty(operatorCellresList[i])) {
                                        ...
                                        $.each(operatorCellresList[i], function (bandNo, band) {
                                            configBand('#' + operator.SysName + '_rru40-band-freq-ul-indicator-' + bandNo, band[0].BandStartUL, band[0].BandStopUL, -110, 0); //rf level input for rru40 is from -110 to 0
                                            configBand('#' + operator.SysName + '_rru40-band-freq-dl-indicator-' + bandNo, band[0].BandStartDL, band[0].BandStopDL, 0, 40);//rf level output for rru40 is from 0 to 40
                                            //scale freq indicator to the band that has maximum range
                                            $('#' + operator.SysName + '_rru40-band-freq-ul-indicator-' + bandNo).css('width', (MAXWIDTH / maxRangeUL) * (band[0].BandStopUL - band[0].BandStartUL));
                                            $('#' + operator.SysName + '_rru40-band-freq-dl-indicator-' + bandNo).css('width', (MAXWIDTH / maxRangeDL) * (band[0].BandStopDL - band[0].BandStartDL));
                                            //set maximum range 110px right from minimum range
                                            var width_ul = $('#' + operator.SysName + '_rru40-band-freq-ul-indicator-' + bandNo).width();
                                            var width_dl = $('#' + operator.SysName + '_rru40-band-freq-dl-indicator-' + bandNo).width();
                                            if(width_dl < 300){
                                                $('.freq-indicator-range-max').css('right', -110); 
                                            }
                                            var html = '<table id="' + operator.SysName + '_band_' + bandNo + '" class="type1 advanced"><tr class="celres-header-row"><th width="8%">ID</th><th>Tag</th><th>Lower Downlink</th><th>Upper Downlink</th><th>Lower Uplink</th><th>Upper Uplink</th><th>RF Level UL</th><th>RF Level DL</th>';
                                            ...
                                            $.each(band, function (key, cellres) {
                                                if (band.length > 0) {
                                                    addCellres(converterUL, '#' + operator.SysName + '_rru40-band-freq-ul-indicator-' + bandNo, cellres.StartUL, cellres.StopUL, operator.SysName + '_' + cellres.ResID);
                                                    addCellres(converterDL, '#' + operator.SysName + '_rru40-band-freq-dl-indicator-' + bandNo, cellres.StartDL, cellres.StopDL, operator.SysName + '_' + cellres.ResID);
