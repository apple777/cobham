...
function ( freqind, api, scheduler, convert, assign, $, _ ) {
    ...
    function startUpdating () {
        ...
        scheduler.add( sec(30), {
             cmd: 'get fqu & get fqd',
             parse: ':n FQU1-center:n :n FQU2-center:n :n FQU3-center:n :n FQU4-center:n :n FQU5-center:n :n FQU6-center:n :n FQU7-center:n :n FQU8-center:n :n FQD1-center:n :n FQD2-center:n :n FQD3-center:n :n FQD4-center:n :n FQD5-center:n :n FQD6-center:n :n FQD7-center:n :n FQD8-center:n',
             onSuccess: function() {
                for (var k in this.parsedResults){
                    this.parsedResults[k] =(this.parsedResults[k]/ 1000000).toFixed(4);
                }
                assign.assign({
                    type:'val',
                    suffix:'-value'
        ...
        var updateFilterConfigs = scheduler.add( sec(30), {
            cmd:'get fcu & get fcd',
        ...
        $( '.attenuation-select').add( '.power-level-select').add( '.config-select' ).change( function () {
            var $this = $( this );
        ...
        $( '.center-config-select' ).change( function () {
            var $this = $( this );
            $this.prop( 'disabled', true );
            api.exe({
                cmd:api.SET( $this.attr( 'data-attribute' ), $this.attr( 'data-number' ), ($this.val()*1000000) ),
                onSuccess: function () {
                    api.exe( updateFrequencies );
                    freqind.update();
                },
