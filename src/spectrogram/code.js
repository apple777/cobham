require([ '/general/general.js', '/js/console.js', '/js/api.js', '/js/scheduler.js', '/js/convert.js', '/js/util.js', '/js/lib/jquery.js', '/js/lib/underscore.js', '/js/lib/jquery-ui.js', '/js/lib/handlebars.js', '/js/handlebars-helpers.js', '/js/led.js','/js/lib/tipsy.js','/js/lib/d3.v3.js' ],
function ( general, console, api, scheduler, convert, util, $, _ ) {
   ...
   var numMeasurementsDefault = 20;

   var maxPoints = 8192;
   var freqParam = 153.6/2;
   var startPointShift = 10;
   ...
   function PlotSpec(){
      ...
      if (bandBandCurrent == "800")
         mirror = 0;    
      else if (bandBandCurrent == "801")
         mirror = 0;
      else if (bandBandCurrent == "1800")
         mirror = 0;    
      else if (bandBandCurrent == "900")
         mirror = 0;
      else if (bandBandCurrent == "2100")
         mirror = 0;
      else if (bandBandCurrent == "400")
         mirror = 0;
      else if (bandBandCurrent == "401")
         mirror = 0;
      ...
      if ((bandBandCurrent == "400") || (bandBandCurrent == "401"))
         startPoint += startPointShift;
      var startPointFft;
      var stopPointFft;
      for (var i = 0; i < maxPoints; i++) {
         if ((startPoint+(i*step))>=$('#start_freq').val()){
            startPointFft = i + maxPoints;
            break;
         }
      }
      for (var i = 0; i < maxPoints; i++) {
         if ((startPoint+(i*step))>=$('#stop_freq').val()){
            stopPointFft = i + maxPoints;
            break;
         }
      }

      api.exe({
       cmd: "fft spec " + fftSource[bandIndex-1] + " " + mirror + " " + dir + " " + $('#num_measurements').val() + " " + startPointFft + " " + stopPointFft + " data.tsv",
       dataType: 'text',
       async: true,
       onSuccess: function (o) {

         var margin = { top: 50, right: 0, bottom: 100, left: 30 },
             width = 100000 - margin.left - margin.right,
             height = 700 - margin.top - margin.bottom,
             gridSize = Math.floor(width / maxPoints),
         ...
         days.length = 0;
         for(var i = 0; i < $('#num_measurements').val(); i++)
            days.push(i);
         times.length = 0;
         for(var i = 0; i <= (stopPointFft - startPointFft); i++)
            times.push(i);
         ...
         document.getElementById('num_measurements').disabled = false;
       },
       onError:function(o){
   ...
   */
   ...
   function GetBands(){
     ...
     $(document).ready(function(){
         ...
         $('#num_measurements').val(numMeasurementsDefault);
         ...
         $("#refresh").click(function(e){
           ...
           document.getElementById('num_measurements').disabled = true;
           PlotSpec();
         })
