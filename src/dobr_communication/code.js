...
function getCmdToken ( $query, cmd, token ) {
    ...
    /**
     * Set Modem section to Enabled / Disabled
     */

    //set all inputs off
    function configMethodDisabled () {
        $(".modem_input").prop("disabled",true);
        setLedColor('#attachmentStatus',null);
        $( '#disEnModem' ).prop( 'checked',false )        
        // todo force gray led. by send true false to getGPRSStatus interval
    }

    //set all inputs on
    function configMethodEnabled () {
        $(".modem_input").prop("disabled",false);
        $( '#disEnModem' ).prop( 'checked',true )        
    }
    ...
    axshCall("GET GPR ENB", function(response,err){
        //console.log(response);
        // init call on load page
        configMethod( response ==1 ? 'enabled' : 'disabled' );

        $( '#disEnModem' ).click( function () {
            configMethod( $( this ).prop( 'checked' ) ? 'enabled' : 'disabled' );
        });
    });
...
/*                    for(var i=0;i<deviceMethods.length;i++){
                    ...
                    var op=$(document.createElement("option")).text("ETH:SNMP:GPRS:SMS");
                    $sel.append(op);
    ...
    function onApply(){
        ...
        if(e.length != 0 && e.length == 4){
            //load configuration from switch
            switchAndAct(e[1]);
            switchAndAct(e[2]);
            switchAndAct(e[3]);
        }
    ...
    function getGPRS()
    {
        ...
        getCmd('#aemMainIpAddressGPRS',"GET GPR ACL");
        getCmd('#aemMainPortGPRS',"GET PPO");
        getCmd('#aemSecondaryIpAddressGPRS',"GET SSC");
        getCmd('#aemSecondaryPortGPRS',"GET SPO");
        getCmd('#aemUsernameGPRS',"GET GPR APN_USER");
        getCmd('#aemPasswordGPRS',"GET GPR APN_PASS");    
    }
    /**
    ...
    function setGPRS(){
        //set Modem inputs
        if($('#accessPointName').val() != ""){
            setCmd( "#accessPointName", "SET GPR APN" );
        }else{
            setCmd( $('#accessPointName').val("-"), "SET GPR APN" );
        }
        if($('#aemUsernameGPRS').val() != "" && $('#aemUsernameGPRS').val() != "-" ){
            setCmd('#aemUsernameGPRS',"SET GPR APN_USER");
        }else{
            setCmd($('#aemUsernameGPRS').val("-"),"SET GPR APN_USER");
        }
        if($('#aemPasswordGPRS').val() != "" && $('#aemPasswordGPRS').val() != "-" ){
            setCmd('#aemPasswordGPRS',"SET GPR APN_PASS");
        }else{
            setCmd($('#aemPasswordGPRS').val("-"),"SET GPR APN_PASS");
        }                           
        //check submitted data is valid
        ...
        //read Modem section to Enabled / Disabled onApply
        if($("#disEnModem").prop("checked")){
            axshCall("SET GPR ENB 1", function ( out, err ){});
        }else{
            axshCall("SET GPR ENB 0", function ( out, err ){});            
        }
    }
    /**
    ...
    function getSMS()
    {
        getCmd('#smsCenterAddress',"GET SMC");
    }
    /**
    ...
    function setSMS(){
        ...
        setCmd( "#smsCenterAddress", "SET SMC" );
    }
    /**
    ...
    function setDualIpConfig(inputAddress, inputPort, secondaryAddress, secondaryPort){
        if(inputAddress&&inputPort){
            //ip + port 1 are ok
            // hide SET ASC - not ok for primary SNMP Manager.
            /*
    ...
    function onDeviceMethodSelectionChange(){
        ...
        $("div.settings").each(function(index, element) {
            ...
            if($(this).attr("id")==selectedPanelId){
                ...
                $('#GPRSsettings').show();
                $('#GPRSsettings').removeClass('hidden');
                //$('#GPSsettings').show();
                //$('#GPSsettings').removeClass('hidden');
            }
            $(this).removeClass('hidden');
 ...
 //       });
    ...
    function readGpsCoordinates() {
      api.exe({
         cmd: 'read_gps_coordinates',
         dataType: 'json',
         async: false,
         onSuccess: function (o) {
            $.each(o.ajaxdata.coordinates, function (i, top) {
              var x = top['x'];
              var y = top['y'];
              $("#gpsCoordinatesX").val(x);
              $("#gpsCoordinatesY").val(y);
              var str = "http://www.google.com/maps/place/"+x+","+y;
              $("#linkMap").attr("href", str);
            })
         },
         error: function(err) {
    ...
    $(document).ready(function(e) {
        $('.interval select').change( function (e)
        {
            $( ".warning-panel" ).show();
        })
        getGPRSStatus();
